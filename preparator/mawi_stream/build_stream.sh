...
if [ "$check_reciprocity" = true ]; then
    for ((index=0; index < ${#all_pcaps[@]}; index++)); do
        ...
        unpigz -c $out_folder/${cur_base_pcap}_tmp.txt.gz | awk '{if ($3 < $2) {print $1" "$3" "$2" "1} else {print $0" "0}}' | sort -k2,3 -t " " -k1,1n -S$memory --parallel=$cpu -T/home/jkaradayi| awk -f $CUR_DIR/reciprocity.awk | gzip -c >> $out_folder/mawi_stream.txt.gz
