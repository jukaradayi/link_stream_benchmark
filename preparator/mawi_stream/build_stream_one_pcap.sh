...
if [ "$check_reciprocity" = true ]; then
    ...
    unpigz -c $out_folder/mawi_stream_tmp.txt.gz | awk '{if ($3 < $2) {print $1" "$3" "$2" "1} else {print $0" "0}}' | sort -k2,3 -t " " -k1,1n | awk -f $CUR_DIR/reciprocity.awk | gzip -c > $out_folder/mawi_stream.txt.gz
